...
export const MobileHeader: React.FC = () => {
  ...
  return (
    <HeaderContainer justifyContent={'center'}>
        ...
        <Toolbar>
          <Stack direction={'row'} alignItems={'center'}>
            ...
            <Typography variant={media.lg ? 'h4' : 'h5'} noWrap component="div">
             <Link href={"/"} color={'inherit'} disableUnderline>{site.title}</Link>
            </Typography>
          </Stack>
          <Box sx={{flexGrow: 1}} />
          <IconButton
      ...
      <MobileMenu open={openMenu} handleClose={() => setMenuOpen(false)} />
      <MobileProfile open={openProfile} handleClose={() => setProfileOpen(false)} />
    </HeaderContainer>
  )
}
